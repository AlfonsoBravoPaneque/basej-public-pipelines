...
// ============================================================================
// BASEJ-RNAPROTEINQC: Single-Cell RNA + Protein (ADT) QC Pipeline (ResolveDOGMA)
// ============================================================================
// Description: RNA QC (STAR alignment, HTSeq quantification, QC metrics) plus
//              antibody (ADT) quantification (bowtie2 barcode alignment) and
//              CPM normalization of ADT counts against matched DNA background.
...
df_long["pipeline"]         = "basej-rnaproteinqc"
...
for _, row in qualimap_df.iterrows():
    ...
    prop_mt = row['prop_mt']
    # Control/empty samples (DNA controls, NTC, beads) may have no protein_coding
    # row in the gene-types summary, leaving NumFeatures as NaN after the left merge.
...
title: "basej-rnaproteinqc v${pipeline_version}"
report_header_info:
...
// ============================================================================
// PROTEIN / ADT QUANTIFICATION PROCESSES (ResolveDOGMA)
// Ported from subworkflows/protein_quant_wf, wired to ECR containers and the
// basej-* publishing model (no publishDir; outputs go through output{}).
// Runs on the same per-biosample FASTQ reads as the RNA backbone.
// ============================================================================

// PROCESS: SEQTK_SEQ — convert FASTQ reads to FASTA
process SEQTK_SEQ {
    tag "${sample_name}"
    ...
    output:
    tuple val(sample_name), path("${sample_name}_R*"), emit: fastas

    script:
    def r2 = reads.size() == 2 ? "seqtk seq -a ${reads[1]} > ${sample_name}_R2.fa" : ""
    """
    seqtk seq -a ${reads[0]} > ${sample_name}_R1.fa
    ${r2}
    """
}

// PROCESS: CUSTOM_CAT_FASTA — concatenate R1+R2 FASTA into one file
process CUSTOM_CAT_FASTA {
    tag "${sample_name}"
    ...
    output:
    tuple val(sample_name), path("${sample_name}_bothReads.fa"), emit: fasta

    script:
    """
    cat ${fasta} > ${sample_name}_bothReads.fa
    """
}

// PROCESS: BOWTIE2_BUILD_S — build a per-sample bowtie2 index from the reads
process BOWTIE2_BUILD_S {
    tag "${sample_name}"
    ...
    output:
    tuple val(sample_name), val("${sample_name}_build"), path("${sample_name}_build*"), emit: genome

    script:
    """
    bowtie2-build-s --threads ${task.cpus} ${fasta} ${sample_name}_build
    """
}

// PROCESS: BOWTIE2 — align the antibody barcode FASTA against the per-sample index
process BOWTIE2 {
    tag "${sample_name}"

    input:
    tuple val(sample_name), val(genome_prefix), path(genome)
    path(barcode)

    output:
    tuple val(sample_name), path("${sample_name}.sam"), emit: alignment

    script:
    """
    bowtie2 -f --all -D 15 -t --threads ${task.cpus} --quiet -x ${genome_prefix} ${barcode} -S ${sample_name}.sam
    """
}

// PROCESS: CUSTOM_COUNT — count reads per antibody barcode
process CUSTOM_COUNT {
    tag "${sample_name}"
    ...
    output:
    path("${sample_name}_counts.txt"), emit: count

    script:
    """
    cut -f 1 ${alignment} | uniq -c | grep -v "\\@" > ${sample_name}_counts.txt
    """
}

// ============================================================================
// PROCESS: PROTEIN_CPM
// Description: Merge per-sample ADT counts with RNA total-read counts, subtract
//              the matched DNA-amplified background, and compute CPM. Emits a
//              wide CPM metadata TSV (for the R heatmap scripts), raw counts TSV,
//              a MultiQC table, and per-biosample long-format Parquet for Athena.
//
// CPM = max(antibody_count - background, 0) / (total_reads / 1e6)
//
// Background resolution (priority order):
//   1. Explicit partner via custom_dna_partner column in input CSV
//   2. Name-token matching: '_R_'->'_D_' or '-R-'->'-D-' on well id (legacy)
//   3. Pooled DNA background: median per-antibody across ALL DNA-fraction
//      samples (used when no specific partner is found — prevents unpaired
//      cells from leaking raw pseudocounts as spurious CPM)
//
// Fraction detection:
//   Primary: custom_molecule_type column (rna / dna, lowercase)
//   Fallback: _R_/_D_ or -R-/-D- name tokens (legacy CSVs)
// ============================================================================
process PROTEIN_CPM {
    tag "protein_cpm"
    ...
    output:
    path("protein_adt_cpm.tsv"), emit: cpm_tsv
    path("protein_adt_raw_counts.tsv"), emit: raw_tsv
    path("protein_adt_cpm_mqc.tsv"), emit: mqc
    path("protein_cpm_summary/workspace=*/workflow_id=*/biosample=*/output.parquet"), emit: parquet
...
# ---------------------------------------------------------------------------
# 1. Read per-sample ADT counts (<sample>_counts.txt: "<count> <antibody>")
# ---------------------------------------------------------------------------
counts = {}          # sample -> {antibody: count}
antibodies = set()
for f in sorted(glob.glob("*_counts.txt")):
    sample = os.path.basename(f)[:-len("_counts.txt")]
    per = {}
    with open(f) as fh:
        for line in fh:
            line = line.strip()
            if not line:
                continue
            parts = line.split(None, 1)   # "<count> <antibody>"
            if len(parts) != 2:
                continue
            cnt, ab = parts
            try:
                per[ab] = per.get(ab, 0) + int(cnt)
            except ValueError:
                continue
            antibodies.add(ab)
    counts[sample] = per
...
if not samples:
    raise SystemExit("PROTEIN_CPM: no *_counts.txt files found")

# ---------------------------------------------------------------------------
# 2. Fraction detection: custom_molecule_type (primary) or name-token (fallback)
# ---------------------------------------------------------------------------
ss = pd.read_csv("${input_csv}")
name_col = "biosampleName" if "biosampleName" in ss.columns else ss.columns[0]

# Guard against NaN / missing values in the metadata columns
has_custom_molecule_type = (
    'custom_molecule_type' in ss.columns
    and ss['custom_molecule_type'].fillna('').str.strip().str.len().gt(0).any()
)

if has_custom_molecule_type:
    print("PROTEIN_CPM fraction source: metadata (custom_molecule_type)")
    fraction_map = dict(zip(
        ss[name_col].astype(str),
        ss['custom_molecule_type'].fillna('').str.strip().str.lower().replace('', 'rna')
    ))
    # Ensure unknown values default to rna
    fraction_map = {k: (v if v in ('rna', 'dna') else 'rna') for k, v in fraction_map.items()}
else:
    print("PROTEIN_CPM fraction source: name-fallback (_R_/_D_)")
    def _is_dna_by_name(name):
        return ("_D_" in name) or ("-D-" in name)
    fraction_map = {s: ('dna' if _is_dna_by_name(s) else 'rna') for s in samples}

# Classify samples
dna_samples = [s for s in samples if fraction_map.get(s, 'rna') == 'dna']
rna_samples = [s for s in samples if fraction_map.get(s, 'rna') != 'dna']
output_samples = rna_samples if rna_samples else samples  # Only emit RNA cells

print(f"  RNA samples: {len(rna_samples)}, DNA samples: {len(dna_samples)}")

# raw count matrix (samples x antibodies)
raw = pd.DataFrame(0, index=samples, columns=antibodies, dtype="int64")
for s in samples:
    for ab, c in counts[s].items():
        raw.at[s, ab] = c

# ---------------------------------------------------------------------------
# 3. Total reads per sample from RNA metrics (denominator)
# ---------------------------------------------------------------------------
reads_by_sample = {}
try:
    m = pd.read_csv("${rna_metrics_tsv}", sep="\\t")
    key_col = "biosample" if "biosample" in m.columns else m.columns[0]
    if "total_reads" in m.columns:
        for _, row in m.iterrows():
            try:
                reads_by_sample[str(row[key_col])] = float(row["total_reads"])
            except (ValueError, TypeError):
                pass
except Exception as e:
    print(f"WARN: could not read RNA metrics: {e}")

# ---------------------------------------------------------------------------
# 4. Partner resolution: custom_dna_partner (primary) or name-token (fallback)
# ---------------------------------------------------------------------------
partner = {}
name_set = set(samples)

# Read custom_dna_partner from metadata if available
has_custom_dna_partner = (
    'custom_dna_partner' in ss.columns
    and ss['custom_dna_partner'].fillna('').str.strip().str.len().gt(0).any()
)

if has_custom_dna_partner:
    partner_meta = dict(zip(
        ss[name_col].astype(str),
        ss['custom_dna_partner'].fillna('').str.strip()
    ))
    for s in samples:
        p = partner_meta.get(s, '')
        # Validate that the named partner actually exists in our count data
        if p and p in name_set:
            partner[s] = p
        elif p:
            print(f"WARN: custom_dna_partner '{p}' for sample '{s}' not found in count data; using pooled background")
            partner[s] = None
        else:
            partner[s] = None
else:
    # Legacy name-token matching
    def well_id(name):
        m = re.search(r"-([0-9]{2}[A-Z])-", name)
    ...
    for n in samples:
        if "-D-" in n:
            w = well_id(n)
            if w:
                dna_by_well.setdefault(w, []).append(n)

    for n in samples:
        p = None
        if "_R_" in n:
            cand = n.replace("_R_", "_D_")
            if cand in name_set:
                p = cand
        elif "-R-" in n:
            w = well_id(n)
            cands = dna_by_well.get(w, []) if w else []
            if cands:
                p = max(cands, key=lambda d: common_suffix_len(n, d))
        partner[n] = p

# ---------------------------------------------------------------------------
# 5. Compute pooled DNA background (median per-antibody across all DNA cells)
#    Used as fallback when a specific partner is absent — prevents unpaired
#    cells from leaking raw pseudocounts as spurious CPM.
# ---------------------------------------------------------------------------
if dna_samples:
    pooled_bg = raw.loc[dna_samples].median(axis=0).astype(float)
    print(f"  Pooled DNA background computed from {len(dna_samples)} DNA samples "
          f"(median per antibody, range: {pooled_bg.min():.1f} - {pooled_bg.max():.1f})")
else:
    # No DNA samples at all: background is zero (best effort)
    pooled_bg = pd.Series(0.0, index=antibodies)
    print("  WARN: No DNA-fraction samples found; pooled background is 0 for all antibodies")

# ---------------------------------------------------------------------------
# 6. Background subtraction + CPM
#    When a specific partner exists, use it. Otherwise use pooled DNA background.
# ---------------------------------------------------------------------------
cpm = pd.DataFrame(0.0, index=samples, columns=antibodies, dtype="float64")
bg_source = {}  # track which background was used per sample for diagnostics

for s in samples:
    p = partner.get(s)
    if p is not None and p in raw.index:
        bg = raw.loc[p].astype(float)
        bg_source[s] = f"partner:{p}"
    else:
        bg = pooled_bg
        bg_source[s] = "pooled_dna_median"
    subtracted = (raw.loc[s].astype(float) - bg).clip(lower=0)
    total_reads = reads_by_sample.get(s, 0.0)
    denom = total_reads / 1e6
    cpm.loc[s] = (subtracted / denom) if denom > 0 else 0.0

# Report background source summary
n_partnered = sum(1 for v in bg_source.values() if v.startswith("partner:"))
n_pooled    = sum(1 for v in bg_source.values() if v == "pooled_dna_median")
print(f"  Background sources: {n_partnered} explicit partner, {n_pooled} pooled median")

# ---------------------------------------------------------------------------
# 7. Write wide TSVs (raw + CPM) for the downstream R heatmap scripts
# ---------------------------------------------------------------------------
raw_out = raw.loc[output_samples].copy()
raw_out.insert(0, "SampleName", raw_out.index)
raw_out["total_reads"] = [reads_by_sample.get(s, 0.0) for s in raw_out.index]
raw_out["dna_partner"] = [partner.get(s) or "" for s in raw_out.index]
raw_out["bg_source"] = [bg_source.get(s, "") for s in raw_out.index]
raw_out.to_csv("protein_adt_raw_counts.tsv", sep="\\t", index=False)

cpm_out = cpm.loc[output_samples].copy()
cpm_out.insert(0, "SampleName", cpm_out.index)
cpm_out["total_reads"] = [reads_by_sample.get(s, 0.0) for s in cpm_out.index]
cpm_out["dna_partner"] = [partner.get(s) or "" for s in cpm_out.index]
cpm_out["bg_source"] = [bg_source.get(s, "") for s in cpm_out.index]
cpm_out.to_csv("protein_adt_cpm.tsv", sep="\\t", index=False)

# ---------------------------------------------------------------------------
# 8. MultiQC table (per-sample ADT summary)
# ---------------------------------------------------------------------------
with open("protein_adt_cpm_mqc.tsv", "w") as fh:
    fh.write("# id: 'protein_adt_cpm'\\n")
    fh.write("# plot_type: 'table'\\n")
    fh.write("# section_name: 'Protein (ADT) CPM'\\n")
    fh.write("# description: 'Per-sample antibody (ADT) quantification: number of antibodies detected, total raw ADT counts, and DNA background partner used for CPM normalization.'\\n")
    fh.write("# pconfig:\\n")
    fh.write("#   id: 'protein_adt_cpm_table'\\n")
    fh.write("sample_name\\tAntibodiesDetected\\tTotalRawADT\\tTotalReads\\tDNA_Partner\\tBG_Source\\n")
    for s in output_samples:
        n_ab = int((raw.loc[s] > 0).sum())
        total_adt = int(raw.loc[s].sum())
        tr = int(reads_by_sample.get(s, 0.0))
        fh.write(f"{s}\\t{n_ab}\\t{total_adt}\\t{tr}\\t{partner.get(s) or 'NA'}\\t{bg_source.get(s, '')}\\n")

# ---------------------------------------------------------------------------
# 9. Per-biosample long-format Parquet for Athena (protein_cpm_summary)
# ---------------------------------------------------------------------------
_str_cols    = ['biosample','antibody','molecule_type','pipeline','pipeline_version',
                'workspace','workflow_id','dataset_id','user','dna_partner','bg_source']
_double_cols = ['cpm','background']
_bigint_cols = ['raw_count','total_reads']
...
for s in output_samples:
    ...
    for ab in antibodies:
        p = partner.get(s)
        if p is not None and p in raw.index:
            bg_val = float(raw.at[p, ab])
        else:
            bg_val = float(pooled_bg[ab])
        rows.append({
            'biosample': s,
            'antibody': ab,
            'raw_count': int(raw.at[s, ab]),
            'background': bg_val,
            'cpm': float(cpm.at[s, ab]),
            'total_reads': int(reads_by_sample.get(s, 0.0)),
            'dna_partner': p or '',
            'bg_source': bg_source.get(s, ''),
    ...
    out_dir = f"protein_cpm_summary/workspace={workspace}/workflow_id={workflow_id}/biosample={s}"
    os.makedirs(out_dir, exist_ok=True)
    pq.write_table(pa.Table.from_pandas(df, preserve_index=False),
                   os.path.join(out_dir, "output.parquet"))

print(f"PROTEIN_CPM: {len(samples)} samples loaded "
      f"({len(output_samples)} reported, {len(samples) - len(output_samples)} DNA background-only) "
      f"x {len(antibodies)} antibodies; "
      f"{n_partnered} explicit partners, {n_pooled} pooled background")
PYEOF
    """
}
...
workflow {
    ...
    // Protein/ADT quant runs on the same per-biosample FASTQ reads (Illumina only).
    // Declared here so it is in scope for both platform branches.
    ch_protein_reads = Channel.empty()
    ...
    // ADT/protein quant uses the same raw reads as the RNA backbone (R + D).
    ch_protein_reads = ch_fastq_input

    // ======================================================================
    // Fraction tagging: determine RNA vs DNA per sample.
    // Primary source: custom_molecule_type column from the metadata CSV
    // (values: rna / dna, lowercase). Fallback: _R_/_D_ or -R-/-D- name
    // tokens for legacy CSVs that lack the column.
    // Used by both the expression path (filter to RNA-only) and PROTEIN_CPM
    // (RNA/DNA classification for background subtraction).
    // ======================================================================
    def fraction_csv_rows = file(params.input_csv).splitCsv(header: true)
    def has_custom_molecule_type = fraction_csv_rows.any { row ->
        row.containsKey('custom_molecule_type') && row.custom_molecule_type?.trim()
    }

    if (has_custom_molecule_type) {
        log.info "fraction source: metadata (custom_molecule_type)"
    } else {
        log.info "fraction source: name-fallback (_R_/_D_)"
    }

    // Build per-sample fraction map: sample -> 'rna' | 'dna'
    def fraction_map = [:]
    fraction_csv_rows.each { row ->
        def name = row.biosampleName?.trim()
        if (!name) return
        if (has_custom_molecule_type) {
            def mt = (row.custom_molecule_type ?: '').trim().toLowerCase()
            fraction_map[name] = (mt == 'dna') ? 'dna' : 'rna'  // default to rna if empty/unknown
        } else {
            // Legacy name-token fallback
            def is_dna = name.contains('_D_') || name.contains('-D-')
            fraction_map[name] = is_dna ? 'dna' : 'rna'
        }
    }

    // DNA-amplified samples are ADT background ONLY. They must not go
    // through the RNA expression path (STAR/HTSeq/Qualimap), so filter them out of
    // the RNA backbone here. They still reach the protein path via ch_protein_reads.
    ch_fastq_rna = ch_fastq_input
        .filter { sample_id, reads -> (fraction_map[sample_id] ?: 'rna') != 'dna' }

        // FASTQ preprocessing pipeline (RNA cells only)
        SEQKIT_SAMPLE(
        ch_fastq_rna.map { sample_id, reads -> [sample_id, reads, params.n_reads] 
        },
    ...
    // ========== Protein / ADT Quantification + CPM (ResolveDOGMA) ==========
    // Runs on the same reads as the RNA backbone. Illumina only for now.
    ch_protein_cpm_tsv     = Channel.empty()
    ch_protein_raw_tsv     = Channel.empty()
    ch_protein_cpm_parquet = Channel.empty()
    ch_protein_mqc         = Channel.empty()

    if (params.skip_protein_quant) {
        log.info "skip_protein_quant=true — running RNA QC only (no ADT quant / CPM)"
    } else if (is_ultima) {
        log.warn "Protein/ADT quantification is Illumina-only; skipping for Ultima (CRAM) input"
    } else {
        // ALL samples (RNA + DNA) go through ADT quantification (bowtie2 barcode
        // alignment). PROTEIN_CPM handles RNA/DNA classification internally using
        // custom_molecule_type / name-tokens, and uses DNA samples only as background.
        // The protein path must NOT depend on ch_read_counts (from SEQKIT_SAMPLE)
        // because both branches run in parallel from the same raw FASTQ inputs.
        ch_protein_reads_nonzero = ch_protein_reads

        SEQTK_SEQ(ch_protein_reads_nonzero)
        CUSTOM_CAT_FASTA(SEQTK_SEQ.out.fastas)
        BOWTIE2_BUILD_S(CUSTOM_CAT_FASTA.out.fasta)
        BOWTIE2(BOWTIE2_BUILD_S.out.genome, file(params.barcode, checkIfExists: true))
        CUSTOM_COUNT(BOWTIE2.out.alignment)

        PROTEIN_CPM(
            CUSTOM_COUNT.out.count.collect(),
            RNA_QC_PLOTS.out.summary_tsv,
            file(params.input_csv, checkIfExists: true),
            params.workspace,
            params.workflow_id,
            params.dataset_id,
        ...
        ch_protein_cpm_tsv     = PROTEIN_CPM.out.cpm_tsv
        ch_protein_raw_tsv     = PROTEIN_CPM.out.raw_tsv
        ch_protein_cpm_parquet = PROTEIN_CPM.out.parquet
        ch_protein_mqc         = PROTEIN_CPM.out.mqc
    }
    ...
    protein_adt_cpm = channel.empty()
        .mix(ch_protein_cpm_tsv)
        .mix(ch_protein_raw_tsv)
        .collect()
        .ifEmpty([])

    protein_cpm_summary = ch_protein_cpm_parquet
}
...
output {
    ...
    protein_adt_cpm {
        path "workflow_outputs/${params.workspace}/${params.workflow_id}/protein_cpm"
        tags workspace: params.workspace,
             dataset_id: params.dataset_id,
             workflow_id: params.workflow_id,
             pipeline: workflow.manifest.name,
             molecule_type: "protein",
             artifact: "protein_adt_cpm"
    }
